...
class BaselineFinetune(MetaTemplate):
    ...
    def set_forward_both(self,x,is_feature = True):
        return self.set_forward_adaptation_both(x,is_feature); #Baseline always do adaptation
    ...
    def set_forward_adaptation_both(self,x,is_feature = True):
        assert is_feature == True, 'Baseline only support testing with feature'
        z_support, z_query  = self.parse_feature(x,is_feature)
        ...
        if self.loss_type == 'softmax':
            linear_clf = nn.Linear(self.feat_dim*2, self.n_way)
        elif self.loss_type == 'dist':
            linear_clf = backbone.distLinear(self.feat_dim*2, self.n_way)
        linear_clf = linear_clf.cuda()
