...
def save_features_both(model_dct, model_plain, data_loader_dct, data_loader_plain, outfile, params):
    ...
    for i, (x, y) in enumerate(data_loader_dct):
        ...
        if params.method == 'baseline++':
            ...
            feat_size = 640
        if all_feats is None:
            all_feats = f.create_dataset('all_feats', [max_count] + [feat_size * 2], dtype='f')
        all_feats[count:count + feats_dct.size(0), 0:feat_size] = feats_dct.data.cpu().numpy()
    ...
    for i, (x, y) in enumerate(data_loader_plain):
        ...
        if all_feats is None:
            all_feats = f.create_dataset('all_feats',
                                         [max_count] + list(feats_plain.size()[1:] + feats_plain.size()[1:]), dtype='f')
        all_feats[count:count + feats_dct.size(0), feat_size:] = feats_plain.data.cpu().numpy()
        """
...
if __name__ == '__main__':
    ...
    checkpoint_dir_plain = '%s/checkpoints/%s/%s_%s_%sway_%sshot' % (
    configs.save_dir, params.dataset, params.model, params.method, params.test_n_way, params.n_shot)
    if params.method == 'S2M2_R':
        checkpoint_dir_dct = '%s/checkpoints/%s/%s_%s_%sway_%sshot' % (
        configs.save_dir, params.dataset, params.model, params.method, params.test_n_way, params.n_shot)

    else:
        checkpoint_dir_dct = '%s/checkpoints/%s/%sdct_%s_%sway_%sshot' % (
        configs.save_dir, params.dataset, params.model, params.method, params.test_n_way, params.n_shot)
    if params.train_aug:
    ...
    if params.save_iter != -1:
        outfile = os.path.join(checkpoint_dir_plain.replace("checkpoints", "features"),
                               split + "_" + str(params.save_iter) + "_both.hdf5")
    else:
        outfile = os.path.join(checkpoint_dir_plain.replace("checkpoints", "features"),
                               split + "_both.hdf5")
    ...
    data_loader_dct = datamgr.get_data_loader_dct(loadfile, aug=False, filter_size=8)
    ...
    if params.method == 'manifold_mixup' or params.method == 'S2M2_R':
        ...
        state_dct = {k: v for k, v in model_dict_load_dct.items() if k in state_dct}
        ...
        state_plain = {k: v for k, v in model_dict_load_plain.items() if k in state_plain}
