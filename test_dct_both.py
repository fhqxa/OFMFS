...
def feature_evaluation(cl_data_file, model, n_way = 5, n_support = 5, n_query = 15, adaptation = False):
    ...
    if adaptation:
        scores  = model.set_forward_adaptation_both(z_all, is_feature = True)
    else:
        scores  = model.set_forward_both(z_all, is_feature = True)
...
#    checkpoint_dir = '%s/checkpoints/%s/%s_%s_%schannels_%sway_%sshot_aug' %(configs.save_dir, params.dataset, params.model, params.method, params.channels, params.test_n_way, params.n_shot)
    ...
    novel_file = os.path.join( checkpoint_dir.replace("checkpoints","features"), split_str +"_both.hdf5")
