...
INTER_MODE = {'NEAREST': cv2.INTER_NEAREST,
...
#     dct_y = pywt.dwt2(y,'haar')
#     dct_cr = pywt.dwt2(cr, 'haar')
#     dct_cb = pywt.dwt2(cb, 'haar')
#
#     return dct_y, dct_cr, dct_cb


# def transform_dct(img, encoder): #origin
#     if img.dtype != 'uint8':
#         img = np.ascontiguousarray(img, dtype="uint8")
#     img = encoder.encode(img, quality=100, jpeg_subsample=2)
#     dct_y, dct_cb, dct_cr = loads(img)  # 28
#     # print('type of y:', type(dct_y))
#     print(dct_y.shape)
#     return dct_y, dct_cb, dct_cr


def transform_dct(img, encoder):     # 1(*)

    encoder.encode(img, quality=100, jpeg_subsample=2)

    # img_ycrcb = cv2.cvtColor(img, cv2.COLOR_RGB2YCrCb)
    #
    # y, cr, cb = cv2.split(img_ycrcb)  # 分离YCrCb通道
    # cb = cv2.resize(cb, (cb.shape[1] // 2, cb.shape[0] // 2))  # 对Cr Cb进行放缩操作，并调整为原来的一半
    # cr = cv2.resize(cr, (cr.shape[1] // 2, cr.shape[0] // 2))
    # 进行DWT变换
    if img.dtype != 'uint8':
        img = np.ascontiguousarray(img, dtype="uint8")
    img = encoder.encode(img, quality=100, jpeg_subsample=2)
    y, cb, cr = loads(img)  # 28
    coeffs_y = pywt.dwt2(y, 'haar')
    dwt_y, (LH, HL, HH) = coeffs_y
    coeffs_cr = pywt.dwt2(cr, 'haar')
    dwt_cr, (LH, HL, HH) = coeffs_cr
    coeffs_cb = pywt.dwt2(cb, 'haar')
    dwt_cb, (LH, HL, HH) = coeffs_cb

    return dwt_y, dwt_cb, dwt_cr




# image = cv2.imread('image.jpg', cv2.IMREAD_COLOR)
# dwt_y, dwt_cr, dwt_cb = transform_dwt(image)
...
#     coeffs = pywt.wavedec2(matrix, wavelet, level=1)
#
#     dct_x_shape = coeffs[0].shape[0] // size
#     dct_y_shape = coeffs[0].shape[1] // size
...
#             tmp_matrix = coeffs[0][i * size:i * size +, j * size:(j + 1) * size]
#             dct[i, j, :] = tmp_matrix.reshape(-1)
#     return dct

# def matrix2dct(matrix, size, wavelet='haar'):  # do myself
#     matrix = np.int16(matrix)
#     matrix = matrix - 128
#     assert matrix.max() < 128 and matrix.min() >= -128, "dwt matrix should be between -128 to 127!"
#
#     wavelet = pywt.Wavelet(wavelet)
#     coeffs = pywt.wavedec2(matrix, wavelet, level=1)
#
...
#     for i in range(dct_x_shape):
#         for j in range(dct_y_shape):
#             tmp_matrix = coeffs[0][i * size:i * size + size, j * size:(j + 1) * size]
#             dct[i, j, :] = tmp_matrix.reshape(-1)
#
#     return dct

# def matrix2dct(matrix, size):  #origin
#     matrix = np.int16(matrix)
#     matrix = matrix - 128
#     assert matrix.max()<128 and matrix.min()>=-128, "dct matrix should be between -128 to 127!"
#     T = np.zeros((size, size))
...
def matrix2dct(matrix, size):   # 2
    matrix = np.int16(matrix)
    matrix = matrix - 128
    assert matrix.max() < 128 and matrix.min() >= -128, "dwt matrix should be between -128 to 127!"

    # Create the DWT transformation matrix
    T = np.zeros((size, size))
    for i in range(size):
        for j in range(size):
            if i == 0:
                ...
                T[i, j] = math.sqrt(2 / size) * math.sin((2 * j + 1) * i * math.pi / (2 * size))
...
def transform_dct_size(img, encoder,size):
    ...
    dct_y = matrix2dct(y, size, wavelet='haar')
    dct_cb = matrix2dct(cb, size, wavelet='haar')
    dct_cr = matrix2dct(cr, size, wavelet='haar')
