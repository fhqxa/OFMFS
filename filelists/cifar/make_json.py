...
datapath = '/home/d4090/ldq/cifar100/'
pathname = datapath

f = open(pathname + 'splits/bertinetto/test.txt')
classes = f.readlines()
...
for each in classes: 
	...
	files = glob.glob( pathname + 'data/' + each + '/*')
...
f = open(pathname + 'splits/bertinetto/train.txt')
classes = f.readlines()
...
f = open(pathname + 'splits/bertinetto/val.txt')
classes = f.readlines()
